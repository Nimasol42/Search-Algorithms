...
class Plotting:
    def __init__(self, xI, xG, environment, FPS=60, algorithm_name=""):  # algorithm_name be parametera ezafe shod
        self.FPS = FPS
        self.xI, self.xG = xI, xG
        self.algorithm_name = algorithm_name  # gerefteneh esme algorithm
    ...
    def draw_grid(self):
        ...
        # esme algorithm ro nemayesh mideh
        if self.algorithm_name:
            algo_label = self.font.render(f"Algorithm: {self.algorithm_name}", True, (0, 0, 0))
            # Gozaresh dar markaze banner
            self.screen.blit(algo_label, ((self.env.x_range * 20) // 2 - algo_label.get_width() // 2, 5))
        self.screen.fill(
            (245, 245, 245),
